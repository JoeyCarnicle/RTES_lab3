...
volatile int rsrcACnt=0, rsrcBCnt=0, noWait=0, exitnum=0;

void *grabRsrcs(void *threadp);
void exit_routine(int threadIdx);
...
void exit_routine(int threadIdx)
{
	...
	pthread_exit(NULL);
	printf("pthread %d exit", threadIdx);


	if(pthread_mutex_unlock(&rsrcB) == 0){
	     printf("unlock B success\n");}
    	if(pthread_mutex_unlock(&rsrcA) == 0){
	     printf("unlock A success\n");}
	
	usleep(rand());
	
	int rc = pthread_create(&threads[0], NULL, grabRsrcs, (void *)&threadParams[THREAD_1]);
